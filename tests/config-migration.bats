...
EOF
  ...
  # Verify all defaults.json keys are present (49 defaults keys)
...
EOF
  ...
  # Verify flag count is correct (49 total, graduated flags removed)
  run jq 'keys | length' "$TEST_TEMP_DIR/.vbw-planning/config.json"
  [ "$status" -eq 0 ]
  [ "$output" = "49" ]
}
...
@test "EXPECTED_FLAG_COUNT matches defaults.json key count" {
  # Verify session-start.sh has EXPECTED_FLAG_COUNT=49
  SCRIPT_COUNT=$(grep 'EXPECTED_FLAG_COUNT=' "$SCRIPTS_DIR/session-start.sh" | grep -oE '[0-9]+' | head -1)
  [ "$SCRIPT_COUNT" = "49" ]
}
