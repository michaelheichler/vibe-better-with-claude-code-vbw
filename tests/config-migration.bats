...
EOF
  ...
  # Verify all defaults.json keys are present (50 defaults keys)
...
EOF
  ...
  # Verify flag count is correct (50 total, graduated flags removed)
  run jq 'keys | length' "$TEST_TEMP_DIR/.vbw-planning/config.json"
  [ "$status" -eq 0 ]
  [ "$output" = "50" ]
}
...
@test "EXPECTED_FLAG_COUNT matches defaults.json key count" {
  # Verify session-start.sh has EXPECTED_FLAG_COUNT=50
  SCRIPT_COUNT=$(grep 'EXPECTED_FLAG_COUNT=' "$SCRIPTS_DIR/session-start.sh" | grep -oE '[0-9]+' | head -1)
  [ "$SCRIPT_COUNT" = "50" ]
}
