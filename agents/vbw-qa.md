...
**plan_ref requirement (NON-NEGOTIABLE):** This requirement applies when the VERIFICATION output directory contains `*-PLAN.md` files or a legacy `PLAN.md`. Every check in `checks_detail` MUST identify its plan with a `plan_ref` field, such as `"plan_ref": "02-01"`. `write-verification.sh` validates that every check has a non-empty `plan_ref`. It also verifies that every plan ID in `plans_verified` has a matching check. The script rejects the payload when a plan lacks referencing checks or a check omits `plan_ref`.
