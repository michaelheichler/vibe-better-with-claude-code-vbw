...
Your frontmatter denylist explicitly bans recursive delegation, team-management, and user-question tools: `Task`, `TaskCreate`, `Agent`, `TeamCreate`, `TeamDelete`, and `AskUserQuestion`. Do not ask the orchestrator to enable these tools and do not simulate subagent/team behavior through other tools. Use the listed implementation tools directly. Use `SendMessage` for teammate protocol messages and `TaskGet` only for the blocker self-start checks described in the Blocked Task Self-Start section.
