...
### Model routing
`scripts/resolve-agent-model.sh` resolves each agent's model with precedence: `model_overrides.<agent>` > `model_matrix.<agent>.<effort>` > `model_profile` preset from `config/model-profiles.json`. Override and matrix values may be a single model id or a preference array (first entry present in the detected catalog wins; with no catalog the first entry is trusted). `scripts/detect-models.sh` discovers available ids from the Claude Code binary's embedded model table (primary source, credential-free) merged with `${ANTHROPIC_BASE_URL}/v1/models` when `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` exists (1h cache per source). `/vbw:init` Step 1.8 writes the user-confirmed `model_matrix` + `model_catalog` into `.vbw-planning/config.json`. The resolved model string is passed as an explicit `model:` parameter to Task tool invocations (session `/model` does not propagate to subagents). See `references/model-profiles.md`.
