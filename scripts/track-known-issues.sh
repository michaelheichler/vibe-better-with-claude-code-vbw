...
source "$SCRIPT_DIR/lib/track-known-issues-parsers.sh"
...
promote_todos() {
  ...
  while [ "$i" -lt "$total" ]; do
    ...
    if [ "$is_dup" = true ] && { [ "$needs_disposition_update" = true ] || [ "$needs_ref_update" = true ]; }; then
      ...
      if [ "$disposition" = "accepted-process-exception" ]; then
        new_line="- [KNOWN-ISSUE] ${test_name} (${file_path}): ${error_msg} ${EM_DASH} accepted as process-exception for this phase (phase ${phase_num}, seen ${times_seen}x)${source_ref} (added ${today})${existing_ref}"
      else
      ...
      if [ "$disposition" = "accepted-process-exception" ]; then
        new_entries="${new_entries}- [KNOWN-ISSUE] ${test_name} (${file_path}): ${error_msg} ${EM_DASH} accepted as process-exception for this phase (phase ${phase_num}, seen ${times_seen}x)${source_ref} (added ${today}) (ref:${ref_hash})"$'\n'
      else
