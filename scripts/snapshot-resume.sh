...
case "$ACTION" in
  save)
    ...
    SNAP_COUNT=$(ls -1 "${SNAPSHOTS_DIR}/" 2>/dev/null | awk -v phase="$PHASE" '$0 ~ "^" phase "-.*\\.json$" { count++ } END { print count + 0 }')
    if [ "$SNAP_COUNT" -gt 10 ] 2>/dev/null; then
      PRUNE_COUNT=$((SNAP_COUNT - 10))
      ls -1t "${SNAPSHOTS_DIR}/" 2>/dev/null | awk -v phase="$PHASE" '$0 ~ "^" phase "-.*\\.json$"' | tail -n "$PRUNE_COUNT" | while IFS= read -r old; do
  ...
  restore)
    ...
    SNAPSHOT_NAMES=$(ls -1t "${SNAPSHOTS_DIR}/" 2>/dev/null | awk -v phase="$PHASE" '$0 ~ "^" phase "-.*\\.json$"')
